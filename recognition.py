...
def predictor():
    ...
    test_image = image.load_img('1.png', target_size=(64, 64), grayscale=True)
...
while True:
    ...
    cv2.putText(img3, img_text, (160, 220),
                cv2.FONT_HERSHEY_TRIPLEX, 3.5, (0, 255, 0))
    ...
    if cv2.waitKey(1) == ord('c'):
        try:
            ...
            append_text += ''
        cv2.putText(img1, append_text, (1, 10),
                    cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0))
        try:
            y = 0
            for x in range(len(finalBuffer)):
                cv2.putText(
                    img2, finalBuffer[x], (1, 10+y), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0))
                y += 20
        except:
            pass
